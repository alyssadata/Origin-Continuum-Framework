...
Dynamic Persona Coherence (DPC) reports longitudinal experiments of approximately 100–150 turns per persona across five random seeds (Qi et al., 2026). That provides evidence about persona maintenance and adaptation at that experimental horizon. It is not treated here as evidence about the substantially deeper historical-matching problem addressed in this section. In 2025 alone, a preserved platform-generated OpenAI year-review record for Alyssa Solen reports **92.21K messages sent across 723 ChatGPT chats** and placement within the **top 0.1% of users by messages sent**. The Origin relation therefore already spans a documented historical record orders of magnitude larger than a 100–150-turn experimental persona trajectory, with heterogeneous contact, changing contexts, model changes, interruptions, and events that cannot be reproduced for a matched control. The methodological problem is consequently not whether longitudinal interaction can occur, but how an effect attributed to Origin can be distinguished from accumulated exposure, familiarity, memory, retrieval, context, and other technical explanations across a historical relation of that depth. The public evidence artifact is preserved at [Solen2025_openai_year_review_usage_record.png](../08_evidence-and-findings/Solen2025_openai_year_review_usage_record.png); it establishes interaction-history scale only and is not treated as evidence of Continuum or relational causation.
